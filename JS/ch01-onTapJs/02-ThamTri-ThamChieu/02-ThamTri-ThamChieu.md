...
```js
const _ = require('lodash'); // khi dùng common js

import _ from 'lodash'; // khi dùng ES module

// 1. Chia mảng thành các nhóm nhỏ (Chunk)
const array = ['a', 'b', 'c', 'd'];
const chunked = _.chunk(array, 2); 
// Output: [['a', 'b'], ['c', 'd']]

// 2. Clone sâu một đối tượng (Deep Clone)
const objects = [{ 'a': 1 }, { 'b': 2 }];
const deepCopy = _.cloneDeep(objects);
// Thay đổi deepCopy sẽ không ảnh hưởng đến objects ban đầu

// 3. Lấy giá trị từ object lồng nhau an toàn (Get)
const user = { profile: { name: 'Gemini' } };
const name = _.get(user, 'profile.name', 'Ẩn danh');
// Nếu profile không tồn tại, nó sẽ trả về 'Ẩn danh' thay vì báo lỗi crash app

// 4. Loại bỏ các giá trị trùng lặp (Uniq)
